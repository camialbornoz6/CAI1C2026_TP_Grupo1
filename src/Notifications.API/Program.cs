...
Log.Logger = new LoggerConfiguration()
    ...
    .WriteTo.File(new JsonFormatter(), "logs/notifications-api-.json", rollingInterval: RollingInterval.Day)
    .CreateLogger();
        ...
        string mensaje = string.Join("; ",
            context.ModelState.Values
                ...
                .Select(e => string.IsNullOrWhiteSpace(e.ErrorMessage)
                    ? "Los datos de la notificacion son invalidos."
                    : e.ErrorMessage));
...
builder.Services.AddScoped<IUsuariosCliente, UsuariosHttpCliente>();

builder.Services.AddHttpClient("Users.API", client =>
{
    string baseUrl = builder.Configuration.GetValue<string>("ExternalServices:UsersApi:BaseUrl") ?? "http://localhost:5223";
